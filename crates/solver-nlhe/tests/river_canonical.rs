...
// Fixed in A58: `NlheSubgame::apply` now translates `Action::AllIn` into
// `Bet(stack_start)` or `Raise(stack_start)` before pushing to the
// action log. Previously a bare `AllIn` in the log returned (0, 0) from
// `ActionLog::pot_contributions_on`, which re-opened `legal_river_actions`
// in the "no aggression yet" branch → unbounded tree + 30 GB RSS OOM.
#[test]
fn no_brainer_fold() {
...
// Fixed in A58 alongside `no_brainer_fold` — see that test's comment
// for the `AllIn` → `Bet(stack_start)` / `Raise(stack_start)`
// translation that bounds the tree.
#[test]
